...
Dlg::Dlg(wxWindow* parent, RTZroute_pi* ppi) : m_Dialog(parent) {
  this->Fit();
...
void Dlg::Addpoint(TiXmlElement* Route, wxString ptlat, wxString ptlon,
                   ...
                   wxString ptviz, wxString ptradius) {
...
int Dlg::DomExportGPX() {
  ...
  wxFileDialog dlg(this, _("Save in GPX format"), wxEmptyString, wxEmptyString,
...
int Dlg::ExportRT4() {
  ...
  wxString fileName;
  wxFileDialog dlg(this, _("Save in RT4 format"), wxEmptyString, wxEmptyString,
                   _T("RT4 files (*.rt4)|*.rt4|All files (*.*)|*.*"),
                   wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
  if (dlg.ShowModal() == wxID_CANCEL) {
...
void Dlg::OnExport(wxCommandEvent& event) {
  wxFileDialog dlg(
      this, "Select file", wxEmptyString, wxEmptyString,
      "GPX (*.gpx) | *.gpx;*.GPX | RTZ files(*.rtz) | *.rtz;*RTZ | Transas "
      ".rt3 (*.rt3) | *.rt3;*RT3 | Transas .rt4 (*.rt4) | *.rt4;*RT4| Sperry "
      "Visonmaster (*.route) | *.route;*ROUTE",
      wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
  if (dlg.ShowModal() == wxID_OK) {
    if (dlg.GetPath() != wxEmptyString) {
      wxString show = dlg.GetPath();
...
void Dlg::GetPivotInfo(wxRealPoint rtt, wxRealPoint rtp, wxRealPoint rtn,
                       double radius, wxRealPoint& rtpivot,
                       double& internalAngle, wxString& portstbd,
                       wxRealPoint& pb, wxRealPoint& pa) {
  ...
  DistanceBearingMercator_Plugin(
      rtt.x, rtt.y, rtp.x, rtp.y, &bfpc,
      &distanceToWaypoint);  // bearing from previous point to the turn (centre)
  DistanceBearingMercator_Plugin(rtt.x, rtt.y, rtn.x, rtn.y, &bfnc,
                                 &distanceToWaypoint);
  DistanceBearingMercator_Plugin(
      rtp.x, rtp.y, rtt.x, rtt.y, &bfcp,
      &distanceToWaypoint);  // from centre to previous waypoint
  DistanceBearingMercator_Plugin(
      rtn.x, rtn.y, rtt.x, rtt.y, &bfcn,
      &distanceToWaypoint);  // from centre to next waypoint

  double halfangle;

  double directionPivot =
      calcCentreOfPivotDirection(bfpc, bfcn, portstbd, halfangle);

  double distancePivot = radius / sin(halfangle * d2r);
  double rLat, rLon;
  PositionBearingDistanceMercator_Plugin(rtt.x, rtt.y, directionPivot,
                                         distancePivot, &rLat, &rLon);
  ...
  PositionBearingDistanceMercator_Plugin(rtt.x, rtt.y, bfcp, h, &p1LatLon.x,
                                         &p1LatLon.y);
  PositionBearingDistanceMercator_Plugin(rtt.x, rtt.y, bfcn, h, &p2LatLon.x,
                                         &p2LatLon.y);
...
void Dlg::CalculateCurvePoints(wxRealPoint ac1, wxRealPoint ac2,
                               wxRealPoint pivotPoint, double radius,
                               double internalAngle, wxString portstbd) {
  Position my_point;
  ...
  DistanceBearingMercator_Plugin(lonlatAc1.y, lonlatAc1.x, lonlatAcP.y,
                                 lonlatAcP.x, &firstBearing,
                                 &distanceToWaypoint);  // degrees
  DistanceBearingMercator_Plugin(lonlatAc2.y, lonlatAc2.x, lonlatAcP.y,
                                 lonlatAcP.x, &lastBearing,
                                 &distanceToWaypoint);  // degrees
...
void Dlg::OnGenerateRadiusGPX(wxCommandEvent& event) { Calculate(event); }

void Dlg::OnEditGPX(wxCommandEvent& event) { OpenXML(); }

void Dlg::OnUpdateTurn(wxCommandEvent& event) { Update(); }

void Dlg::OnSaveGPX(wxCommandEvent& event) { SaveGPX(); }
...
void Dlg::OnClose(wxCloseEvent& event) { pPlugIn->OnRTZrouteDialogClose(); }
...
bool Dlg::OpenXML() {
  ...
  if (!doc.LoadFile(filename.mb_str())) {
    ...
    if (!strcmp(root->Value(), "rte")) FAIL(_("rte Invalid xml file"));
    ...
    for (TiXmlElement* e = root->FirstChildElement(); e;
      ...
      for (TiXmlElement* f = e->FirstChildElement(); f;
        ...
        if (!strcmp(f->Value(), "rtept")) {
          ...
          my_position.viz =
              wpViz;  // dummy value to make sure there is an entry
          my_position.radius =
              "0.0";  // dummy value to make sure there is an entry
          my_position.planned_speed =
              planned_speed;  // dummy value to make sure there is an entry
          ...
          for (TiXmlElement* g = f->FirstChildElement(); g;
            ...
            if (!strcmp(g->Value(), "extensions")) {
              for (TiXmlElement* h = g->FirstChildElement(); h;
...
int Dlg::ImportRTZ(wxString myFile) {
  ...
  XMLElement* pRouteNameElement =
      xmlDoc.FirstChildElement("route")->FirstChildElement("routeInfo");
  ...
  XMLElement* pWaypointsElement =
      xmlDoc.FirstChildElement("route")->FirstChildElement("waypoints");
  if (pWaypointsElement == nullptr) return XML_ERROR_PARSING_ELEMENT;

  XMLElement* pListWaypointsElement =
      pWaypointsElement->FirstChildElement("waypoint");
  ...
  while (pListWaypointsElement != nullptr) {
    ...
    XMLElement* pListPositionElement =
        pListWaypointsElement->FirstChildElement("position");
    ...
    while (pListPositionElement != nullptr) {
      ...
      pListPositionElement = pListPositionElement->NextSiblingElement(
          "position");  // stop the loop when position empty
    }

    pListWaypointsElement = pListWaypointsElement->NextSiblingElement(
...
void Dlg::Calculate(wxCommandEvent& event) {
  ...
  if (!step_string.ToDouble(&value)) {
    wxLogMessage("RTZroute: error obtaining turn step");
  }
  turnStep = value;
  ...
  wxFileDialog dlg(this, _("Export RTZroute Positions in GPX file as"),
  ...
  s = dlg.GetPath();
  if (dlg.GetPath() == wxEmptyString) {
    if (dbg) printf("RTZroute: Empty Path\n");
  }
  ...
  for (i = 0; i < n; i++) {  // n is number of routepoints
    ...
    if (i == 0) {  // First F is a routepoint
      ...
      if (rad[i] > 0.0) {
        ...
        GetPivotInfo(rtt, rp, rn, rad[i], pivotPoint, internal_angle, portstbd,
                     ac1, ac2);
        CalculateCurvePoints(ac1, ac2, pivotPoint, rad[i], internal_angle,
                             portstbd);
      } else {
        my_point.lat = wxString::Format(wxT("%f"), latN[i]);
        my_point.lon = wxString::Format(wxT("%f"), lonN[i]);
  ...
  for (std::vector<Position>::iterator itOut = my_points.begin();
    ...
    if ((*itOut).routepoint == 1) {
      Addpoint(Route, wxString::Format(wxT("%f"), lati),
               ...
               _T("diamond"), _T("WPT"), (*itOut).viz, s_radius);
    } else {
      if ((*itOut).routepoint == 0) {
        Addpoint(Route, wxString::Format(wxT("%f"), lati),
                 wxString::Format(wxT("%f"), loni), (*itOut).wpName,
                 _T("square"), _T("WPT"), (*itOut).viz, s_radius);
      }
    }
  }
  my_points.clear();
  ...
  this->m_listBoxWaypoints->Clear();  // prevent user making a second export of
                                      // the same waypoints ?????
  my_positions.clear();
  ClearTextboxes();
}
...
void Dlg::Update() {
  ...
  for (std::vector<Position>::iterator itOut = draftPositions.begin();
       itOut != draftPositions.end(); itOut++) {
    // wxMessageBox((*it).lat, _T("*it.lat"));
...
void Dlg::SaveGPX() {
  ...
  TiXmlElement* RouteName = new TiXmlElement("name");
  TiXmlText* text4 = new TiXmlText(this->m_Route->GetValue().ToUTF8());
  // TiXmlText * textSpeed = new
  // TiXmlText(this->m_Speed_PS->GetValue().ToUTF8());
  ...
  for (std::vector<Position>::iterator itOut = my_positions.begin();
    ...
    Addpoint(Route, wxString::Format(wxT("%f"), lati),
             wxString::Format(wxT("%f"), loni), (*itOut).wpName, (*itOut).wpSym,
             _T("WPT"), s_viz, s_radius);
  }
